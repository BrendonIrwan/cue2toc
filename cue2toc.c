...
check_once(enum command cmd, char *s, enum scope sc)
{
	if (s[0] == '\0')
		return;
	fprintf(stderr, "%s:%s:%ld: %s allowed only once", prog, fname,
		line, cmds[cmd]);
	switch (sc) {
...
check_cutting_binary(struct trackspec *tr)
{
	...
	if (tr->pregap_data_from_file) {
		if (tr->pregap < tr->start)
			err_cue("TOC format does not allow cutting binary "
...
read_cue(const char *cuefile)
{
	...
	while ((cmd = get_command(f)) != TRACK) {
		...
		case CATALOG:
			...
			if (n != 13)
				err_cue("Catalog number must be 13 "
					 "characters long");
			break;
		case TITLE:
	...
	while (cmd != END) {
		...
		case TRACK:
			...
			if (track->mode == AUDIO && filetype == BINARY
				...
				err_cue("File and track type mismatch");
			*/
		...
		case INDEX:
			...
			case 0:
				...
				if (track->pregap != -1)
					err_cue("PREGAP allowed only once "
						 "per track");
			...
			case 1:
				if (track->start != -1)
					err_cue("Each index allowed only "
						 "once per track");
				if (get_string(f, timecode_buffer,
					       TCBUFLEN - 1) < 1)
...
determine_session_type(struct trackspec *list)
{
	...
	while (track != NULL) {
		...
		default:	/* should never get here */
			err_quit("Dont know how this could happen, but here "
...
/* Write a track to the file f. The arguments i and l are the indentation
   ...
   cdtext is zero. */
static void
write_track(struct trackspec *tr, FILE *f, int i, int l)
{
	char timecode_buffer[TCBUFLEN];
	long start = 0, len = 0;
	int j = 0;
	...
	default:	err_quit("Unknown track mode"); /* cant get here */
	}
	...
	if (tr->pregap > 0 && !tr->pregap_data_from_file) {
	...
	/* Indexes */
 	while (tr->indexes[j] != -1 && j < NUM_OF_INDEXES) {
 		if (fr2tc(timecode_buffer, tr->indexes[j++] - start) == -1)
...
void
write_toc(const char *tocfile, struct cuesheet *cs)
{
	FILE *f = stdout;
	...
	cdtext = contains_cdtext(cs) && cdtext;
	if (tocfile != NULL)
		if ((f = fopen(tocfile, "w")) == NULL)
			err_sys("Could not open file \"%s\" for writing",
				tocfile);
	...
	default:	err_quit("Should never get here");
	}
